...
- **Report Generation** — `Project_Report.docx` with embedded charts, via dashboard button or CLI
...
```bash
streamlit run upi_fraud_analysis.py
```
...
```bash
python upi_fraud_analysis.py --pipeline
```

This generates all charts, tables, results, and `Project_Report.docx` without opening a browser.
...
UPI-Transaction-Risk-Fraud-Analytics/
│
├── upi_fraud_analysis.py     ← Single Python file: analysis + dashboard + report
├── requirements.txt
├── README.md
├── Project_Report.docx       ← Generated by pipeline or dashboard button
│
├── data/
